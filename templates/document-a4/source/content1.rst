...
.. This is how you include something from a directory above

.. include:: ../library/common/mathsymbols.rst
...
This is how you label and include figures.

.. figure:: _images/phdia.png
   :name: diagram
   :width: 100%

   Once I created this for Wikipedia


As shown in :numref:`diagram` entropy is blue ;)


And you can cite some book like :cite:`IPCC2014` and let bibtex manage the references.
Look at the end of this Document. The bibtex entries are in library/common.
...
A.B.C.D SubSubSubChapter
---------------------------------



.. index:: Too many Sub Sub Sub is bad structure


A.B.C.D.E SubSubSubSubChapter
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^


A maxdepth of five is defined in the index file
...
**This one comes from https://github.com/openearth/xbeach-docs**
